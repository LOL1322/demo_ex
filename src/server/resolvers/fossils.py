...
def create_fossil(fossil: Fossil):
    return base_worker.execute_query(
        query="INSERT INTO fossils(expedition_id, name, date_of_discovery, age, quantity) VALUES (?, ?, ?, ?, ?) RETURNING id",
        args=(fossil.expedition_id, fossil.name, fossil.date, fossil.age, fossil.quantity))


def get_fossil(fossil_id: int):
    return base_worker.execute_query(
        query="SELECT id, expedition_id, name, date_of_discovery, age, quantity FROM fossils WHERE id = ?",
        args=(fossil_id,))


def get_all_fossil():
    return base_worker.execute_query(
        query="SELECT id, expedition_id, name, date_of_discovery, age, quantity FROM fossils",
        fetchone=False)


def update_fossil(fossil_id: int, new_data: Fossil):
    return base_worker.execute_query(
        query="UPDATE fossils SET expedition_id=?, name=?, date_of_discovery=?, age=?, quantity=? WHERE id=?",
...
def delete_fossil(fossil_id: int):
    return base_worker.execute_query(query="DELETE FROM fossils WHERE id=? ",
                                     args=(fossil_id,))
